...
void put(char *id, node_t *nptr) {
    entry_t* ent = init_entry(id, nptr);
    unsigned long index = hash_function(id);
    if(var_table->entries[index] == NULL){
        var_table->entries[index] = ent;
    }else{
        entry_t* tmp = var_table->entries[index];
        while (tmp->next != NULL){
            if(strcmp(tmp->id, id) == 0){
                tmp->type = ent->type;
                if(ent->type == INT_TYPE){
                    tmp->val.ival = ent->val.ival;
                }else{
                    tmp->val.sval = ent->val.sval;
                }
                return;
            }else{
                tmp = tmp->next;
